...
function showMeasurements(itemId){const i=project().items.find(x=>x.id===itemId),ms=period().measurements.filter(x=>x.itemId===itemId);$('measureListTitle').textContent=i.name+' · Metraj detayları';$('measureList').innerHTML=ms.length?ms.map(m=>`<div class="row"><div class="row-main"><b>${num(qtyOf(m))} ${i.unit}</b><div class="meta">${esc(m.section||'Bölüm yok')} · ${esc(m.description||'Açıklama yok')}</div></div><div class="row-actions"><button type="button" class="mini edit-measure" data-id="${m.id}">Düzenle</button><button type="button" class="mini danger delete-measure" data-id="${m.id}">Sil</button></div></div>`).join(''):'<div class="empty">Metraj kaydı yok.</div>';openDialog('measureListDialog')}

$('newProjectBtn').onclick=()=>openProject();$('editProjectBtn').onclick=()=>openProject(activeProjectId);$('deleteProjectBtn').onclick=()=>{const p=project();if(p&&confirm(p.name+' projesi, hakedişleri ve bütün kayıtları silinsin mi?')){db.projects=db.projects.filter(x=>x.id!==p.id);activeProjectId=db.projects[0]?.id||null;activePeriodId=db.projects[0]?.periods?.[0]?.id||null;persist('Proje silindi');showPage('dashboard')}};$('newItemBtn').onclick=()=>openItem();$('newPeriodBtn').onclick=()=>openPeriod();$('editPeriodBtn').onclick=()=>openPeriod(activePeriodId);$('deletePeriodBtn').onclick=()=>{const p=project(),h=period();if(h&&confirm('Hakediş No '+h.no+' ve bütün metrajları silinsin mi?')){p.periods=p.periods.filter(x=>x.id!==h.id);activePeriodId=sortedPeriods(p).at(-1)?.id||null;persist('Hakediş silindi');showPage('project')}};$('addMeasurementBtn').onclick=()=>project().items.length?openMeasurement():alert('Önce iş kalemi ekleyin.');$('directEntryBtn').onclick=openDirect;$('projectSearch').oninput=renderDashboard;$('undoBtn').onclick=undo;$('reportProject').onchange=fillReportPeriods;['mCount','mWidth','mLength','mHeight','mFactor','mManual','mManualEnabled'].forEach(id=>$(id).addEventListener('input',previewMeasurement));
...
function groupedRows(rows){const groups=[];rows.forEach(r=>{const name=r.item.group?.trim()||'GENEL';let g=groups.find(x=>x.name===name);if(!g){g={name,rows:[]};groups.push(g)}g.rows.push(r)});return groups}
function exportPdf(){
  ...
  const groups=groupedRows(c.rows),head=(title)=>({table:{widths:['*','auto'],body:[[{text:p.workName||'BAKIM ONARIM TADİLAT VE İMALAT HAKEDİŞİ',bold:true,alignment:'center',margin:[0,18,0,18]},{text:'HAKEDİŞ NO : '+h.no,bold:true,alignment:'center',margin:[18,18,18,18]}]]},layout:'lightHorizontalLines'});
  const icmalBody=[[{text:'SIRA NO',bold:true,alignment:'center'},{text:'İŞ KALEMİNİN ADI VE KISA AÇIKLAMASI',bold:true,alignment:'center'},{text:'TUTAR',bold:true,alignment:'center'}]];
  groups.forEach((g,gi)=>{icmalBody.push([{text:String(gi+1),color:'#d70000',bold:true,alignment:'center'},{text:g.name,color:'#d70000',bold:true},{text:money(g.rows.reduce((s,r)=>s+r.currentAmount,0)),bold:true,alignment:'right'}]);g.rows.forEach(r=>icmalBody.push(['',{text:r.item.name},{text:money(r.currentAmount),alignment:'right'}]))});
  icmalBody.push(['',{text:'TOPLAM',bold:true},{text:money(c.grossBeforeDiscount),bold:true,alignment:'right'}]);
  const carsafBody=[[{text:'SIRA NO',bold:true,alignment:'center'},{text:'POZ NO',bold:true,alignment:'center'},{text:'YAPILAN İŞİN CİNSİ',bold:true,alignment:'center'},{text:'BİRİM',bold:true,alignment:'center'},{text:'ÖNCEKİ HAKEDİŞ TOPLAMI',bold:true,alignment:'center'},{text:'BU HAKEDİŞ İMALATI',bold:true,alignment:'center'},{text:'TOPLAM İMALAT MİKTARI',bold:true,alignment:'center'},{text:'BİRİM FİYATLAR',bold:true,alignment:'center'},{text:'TUTAR',bold:true,alignment:'center'}]];
  groups.forEach((g,gi)=>{carsafBody.push([{text:String(gi+1),color:'#d70000',bold:true,alignment:'center'},'',{text:g.name,color:'#d70000',bold:true},'','','','','','']);g.rows.forEach(r=>carsafBody.push(['',r.item.code||'',r.item.name,r.item.unit,{text:num(r.previous),alignment:'center'},{text:num(r.current),alignment:'center'},{text:num(r.total),alignment:'center'},{text:money(r.item.unitPrice),alignment:'right'},{text:money(r.currentAmount),alignment:'right'}]))});
  carsafBody.push(['','','','','','','',{text:'TOPLAM',bold:true,alignment:'right'},{text:money(c.grossBeforeDiscount),bold:true,alignment:'right'}]);
  ...
  const content=[
    ...
    head('İCMAL'),{text:'GENEL TABLO',style:'sheetTitle'},{table:{headerRows:1,widths:[42,'*',100],body:icmalBody},layout:'grid',fontSize:8},{table:{widths:['*','*'],body:[[{text:'TAŞERON\n\n\n'+p.contractor,alignment:'center',bold:true},{text:'İŞVEREN\n\n\n'+p.employer,alignment:'center',bold:true}]]},layout:'grid',margin:[0,8,0,0]},
    Object.assign(head('ÇARŞAF'),{pageBreak:'before'}),{text:'GENEL',style:'sheetTitle'},{table:{headerRows:1,widths:[28,38,'*',35,48,48,48,58,68],body:carsafBody},layout:'grid',fontSize:6.5},{table:{widths:['*','*'],body:[[{text:'TAŞERON\n\n\n'+p.contractor,alignment:'center',bold:true},{text:'İŞVEREN\n\n\n'+p.employer,alignment:'center',bold:true}]]},layout:'grid',margin:[0,8,0,0]},
  ...
  const doc={pageSize:'A4',pageOrientation:'landscape',pageMargins:[22,22,22,22],defaultStyle:{font:'Roboto',fontSize:9,color:'#111'},content,styles:{brand:{fontSize:9,bold:true,color:'#16a293',characterSpacing:2},title:{fontSize:20,bold:true,color:'#102a43',margin:[0,4,0,2]},project:{fontSize:12,bold:true,color:'#536b7b'},heading:{fontSize:13,bold:true,color:'#102a43',margin:[0,12,0,6]},sheetTitle:{fontSize:14,bold:true,alignment:'center',margin:[0,8,0,8]},label:{fontSize:8,color:'#718290'}}};
...
async function exportExcel(){
  if(typeof ExcelJS==='undefined')return alert('Excel motoru yüklenemedi.');const{p,h,c}=reportSelection();if(!p||!h)return alert('Proje ve hakediş seçin.');
  const wb=new ExcelJS.Workbook();wb.creator='MRL Hakediş';
  ...
  const addSheet=(name,rows)=>{const ws=wb.addWorksheet(name);ws.addRows(rows);ws.getRow(1).font={bold:true,color:{argb:'FFFFFFFF'}};ws.getRow(1).fill={type:'pattern',pattern:'solid',fgColor:{argb:'FF102A43'}};ws.columns.forEach(col=>col.width=18);return ws};
  const border={top:{style:'thin'},left:{style:'thin'},bottom:{style:'thin'},right:{style:'thin'}},center={vertical:'middle',horizontal:'center',wrapText:true},currency='#,##0.00 "TL"';
  const setup=(ws,lastCol)=>{ws.pageSetup={paperSize:9,orientation:'landscape',fitToPage:true,fitToWidth:1,fitToHeight:0,margins:{left:.25,right:.25,top:.35,bottom:.35,header:.15,footer:.15}};ws.views=[{showGridLines:false}];ws.mergeCells(`A1:${String.fromCharCode(64+Math.floor(lastCol/2))}2`);ws.mergeCells(`${String.fromCharCode(65+Math.floor(lastCol/2))}1:${String.fromCharCode(64+lastCol)}2`);ws.getCell('A1').value=p.workName||'BAKIM ONARIM TADİLAT VE İMALAT HAKEDİŞİ';ws.getCell('A1').font={bold:true,size:12};ws.getCell('A1').alignment=center;const hc=ws.getCell(`${String.fromCharCode(65+Math.floor(lastCol/2))}1`);hc.value='HAKEDİŞ NO : '+h.no;hc.font={bold:true,size:11};hc.alignment=center;ws.getRows(1,2).forEach(row=>row.height=26)};
  const styleRange=(ws,from,to,cols)=>{for(let r=from;r<=to;r++)for(let c2=1;c2<=cols;c2++){const cell=ws.getCell(r,c2);cell.border=border;cell.alignment=cell.alignment?.horizontal?cell.alignment:{vertical:'middle',wrapText:true}}};
  const icmal=wb.addWorksheet('İcmal');setup(icmal,3);icmal.mergeCells('A3:C3');icmal.getCell('A3').value='GENEL TABLO';icmal.getCell('A3').font={bold:true,size:14};icmal.getCell('A3').alignment=center;icmal.addRow(['SIRA NO','İŞ KALEMİNİN ADI VE KISA AÇIKLAMASI','TUTAR']);icmal.getRow(4).height=38;icmal.getRow(4).eachCell(x=>{x.font={bold:true};x.alignment=center});let ir=5;groupedRows(c.rows).forEach((g,gi)=>{const gr=icmal.addRow([gi+1,g.name,g.rows.reduce((s,r)=>s+r.currentAmount,0)]);gr.font={bold:true,color:{argb:'FFFF0000'}};gr.getCell(3).numFmt=currency;ir++;g.rows.forEach(r=>{const row=icmal.addRow(['',r.item.name,r.currentAmount]);row.getCell(3).numFmt=currency;ir++})});const it=icmal.addRow(['','TOPLAM',c.grossBeforeDiscount]);it.font={bold:true};it.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFFC000'}};it.getCell(3).numFmt=currency;ir++;icmal.mergeCells(`A${ir}:B${ir+2}`);icmal.getCell(`A${ir}`).value='TAŞERON\n\n'+p.contractor;icmal.getCell(`A${ir}`).alignment=center;icmal.getCell(`A${ir}`).font={bold:true};icmal.getCell(`C${ir}`).value='İŞVEREN\n\n'+p.employer;icmal.mergeCells(`C${ir}:C${ir+2}`);icmal.getCell(`C${ir}`).alignment=center;icmal.getCell(`C${ir}`).font={bold:true};icmal.columns=[{width:9},{width:62},{width:24}];styleRange(icmal,1,ir+2,3);
  const carsaf=wb.addWorksheet('Çarşaf');setup(carsaf,9);carsaf.mergeCells('A3:I3');carsaf.getCell('A3').value='GENEL';carsaf.getCell('A3').font={bold:true,size:14};carsaf.getCell('A3').alignment=center;carsaf.addRow(['SIRA NO','POZ NO','YAPILAN İŞİN CİNSİ','BİRİM','ÖNCEKİ HAKEDİŞ TOPLAMI','BU HAKEDİŞ İMALATI','TOPLAM İMALAT MİKTARI','BİRİM FİYATLAR','TUTAR']);carsaf.getRow(4).height=52;carsaf.getRow(4).eachCell(x=>{x.font={bold:true};x.alignment=center});let cr=5;groupedRows(c.rows).forEach((g,gi)=>{const gr=carsaf.addRow([gi+1,'',g.name,'','','','','','']);gr.font={bold:true,color:{argb:'FFFF0000'}};cr++;g.rows.forEach(r=>{const row=carsaf.addRow(['',r.item.code||'',r.item.name,r.item.unit,r.previous,r.current,r.total,r.item.unitPrice,r.currentAmount]);[5,6,7].forEach(n=>row.getCell(n).numFmt='0.000');[8,9].forEach(n=>row.getCell(n).numFmt=currency);cr++})});const ct=carsaf.addRow(['','','','','','','','TOPLAM',c.grossBeforeDiscount]);ct.font={bold:true};ct.fill={type:'pattern',pattern:'solid',fgColor:{argb:'FFFFC000'}};ct.getCell(9).numFmt=currency;cr++;carsaf.mergeCells(`A${cr}:D${cr+2}`);carsaf.getCell(`A${cr}`).value='TAŞERON\n\n'+p.contractor;carsaf.getCell(`A${cr}`).alignment=center;carsaf.getCell(`A${cr}`).font={bold:true};carsaf.mergeCells(`E${cr}:I${cr+2}`);carsaf.getCell(`E${cr}`).value='İŞVEREN\n\n'+p.employer;carsaf.getCell(`E${cr}`).alignment=center;carsaf.getCell(`E${cr}`).font={bold:true};carsaf.columns=[{width:8},{width:10},{width:40},{width:9},{width:14},{width:14},{width:14},{width:17},{width:19}];styleRange(carsaf,1,cr+2,9);
  addSheet('Kapak',cover);addSheet('Metrajlar',measures);
  const out=await wb.xlsx.writeBuffer();shareFile('MRL-Hakedis-'+h.no+'.xlsx',out,'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
}
function importLegacy(workbook,fileName='Excel Hakediş Projesi'){
  const sheet=workbook.getWorksheet('ÇARŞAF')||workbook.getWorksheet('Çarşaf')||workbook.worksheets.find(s=>s.name.toLocaleUpperCase('tr-TR').includes('ÇARŞAF'));if(!sheet)throw Error('ÇARŞAF sayfası bulunamadı');
  const cover=workbook.getWorksheet(' kapak')||workbook.getWorksheet('kapak');const val=(s,cell)=>{let v=s?.getCell(cell).value;if(v&&typeof v==='object')v=v.result??v.text??'';return v??''};
  const p={id:uid(),name:fileName.replace(/\.xlsx?$/i,'')||'Excel Hakediş Projesi',employer:String(val(cover,'J11')||'İŞVEREN'),contractor:String(val(cover,'N38')||'MERAL MOBİLYA'),workName:String(val(cover,'N35')||'BAKIM ONARIM TADİLAT VE İMALAT'),contractDate:'',deliveryDate:'',endDate:'',createdAt:new Date().toISOString(),items:[],periods:[]};
...
$('importExcel').onchange=async e=>{try{const f=e.target.files[0];if(!f)return;const wb=new ExcelJS.Workbook();await wb.xlsx.load(await f.arrayBuffer());importLegacy(wb,f.name)}catch(err){alert('Excel aktarılamadı: '+err.message)}e.target.value=''};
