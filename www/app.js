...
const ids=['totalPay','totalOvertime','totalAdvance','totalRemaining','summaryList','employeeList','recordEmployeeFilter','recordEmployee','recordMonth','recordList','employeeDialog','employeeDialogTitle','employeeId','employeeName','employeeSalary','employeeHours','recordDialog','recordForm','recordDate','recordOvertimeHours','recordAdvance','recordMeal','recordNote','paymentDialog','paymentForm','paymentEmployeeId','paymentPerson','paymentAmount','employeeForm','prevMonth','nextMonth','addEmployeeBtn','addRecordBtn','backupBtn','exportJson','importJson','exportCsv'];
const el=Object.fromEntries(ids.map(id=>[id,byId(id)]));
const {totalPay,totalOvertime,totalAdvance,totalRemaining,summaryList,employeeList,recordEmployeeFilter,recordEmployee,recordMonth,recordList,employeeDialog,employeeDialogTitle,employeeId,employeeName,employeeSalary,employeeHours,recordDialog,recordForm,recordDate,recordOvertimeHours,recordAdvance,recordMeal,recordNote,paymentDialog,paymentForm,paymentEmployeeId,paymentPerson,paymentAmount,employeeForm,prevMonth,nextMonth,addEmployeeBtn,addRecordBtn,backupBtn,exportJson,importJson,exportCsv}=el;
...
function load(){try{return JSON.parse(localStorage.getItem(STORE))||clone(initial)}catch{return clone(initial)}}
...
employeeForm.addEventListener('submit',e=>{e.preventDefault();const id=employeeId.value, obj={id:id||uid(),name:employeeName.value.trim(),salary:+employeeSalary.value,normalHours:+employeeHours.value};if(id)db.employees[db.employees.findIndex(x=>x.id===id)]=obj;else db.employees.push(obj);save();employeeDialog.close();toast('Personel kaydedildi')});
recordForm.addEventListener('submit',e=>{e.preventDefault();db.records.push({id:uid(),employeeId:recordEmployee.value,date:recordDate.value,overtimeHours:+recordOvertimeHours.value,advance:+recordAdvance.value,meal:+recordMeal.value,note:recordNote.value.trim()});save();recordDialog.close();recordForm.reset();recordDate.value=new Date().toISOString().slice(0,10);toast('Günlük kayıt eklendi')});
paymentForm.addEventListener('submit',e=>{e.preventDefault();db.payments=db.payments.filter(p=>!(p.employeeId===paymentEmployeeId.value&&p.period===currentPeriod()));db.payments.push({id:uid(),employeeId:paymentEmployeeId.value,period:currentPeriod(),amount:+paymentAmount.value});save();paymentDialog.close();toast('Ödeme kaydedildi')});
